...
def load_hifigan_vocoder(language, gender, device):
    ...
    vocoder_config = f"vocoder/{gender}/aryan/hifigan/hindi_rev1/correct/config.json"
    vocoder_generator = f"vocoder/{gender}/aryan/hifigan/hindi_rev1/correct/generator"
...
def load_fastspeech2_model(language, gender, device, model):
    ...
    feat=f"{model}/feats_stats.npz"
    pitch=f"{model}/pitch_stats.npz"
    energy=f"{model}/energy_stats.npz"
...
def text_synthesis(language, gender, sample_text, vocoder, MAX_WAV_VALUE, device, alpha):
    ...
    with torch.no_grad():
        ...
        model = load_fastspeech2_model(language, gender, device, model_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ipu", type=int, default=0, help="IPU Type")
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for sample_text in result:
            ...
            print("preprocessed_text: ", preprocessed_text)
            # Transform preprocessed text according to the IPU Type
            audio_text = []
            if args.ipu == 0:
                audio_text.append('$' + preprocessed_text[1:-1] + '.')
            elif args.ipu == 1:
                audio_text.append('$' + preprocessed_text[1:-1] + '1')
            elif args.ipu == 2:
                audio_text.append('2' + preprocessed_text[1:-1] + '3')
            else:
                audio_text.append('4' + preprocessed_text[1:-1] + '.')

            audio_text = "".join(audio_text)
            print("audio_text: ", audio_text)
            # Generate audio from the preprocessed text using a text-to-speech synthesis function
            audio = text_synthesis(args.language, args.gender, audio_text, vocoder, MAX_WAV_VALUE, device, args.alpha)
